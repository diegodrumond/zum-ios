...
class ListAddressViewController : BaseViewController, AddAddressViewDelegate, UITableViewDelegate, UITableViewDataSource{
    ...
    func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        ...
        let address = addressData.data[indexPath.row]
        performSegueWithIdentifier("SegueCheckListItem", sender: address)
        
    }
    
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        if segue.identifier == "SegueCheckListItem" {
            let checkListViewController = segue.destinationViewController as! CheckListViewController
            let address  = sender as! Address
            checkListViewController.address = address
        }
